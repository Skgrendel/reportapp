        ...
        return [
            SelectFilter::make('Ciclos')
                ...
                ->filter(function (Builder $builder, $value) {
                    if ($value === '1') {
                        $builder->where('ciclo.ciclo', '1001');
                    } elseif ($value === '2') {
                        $builder->where('ciclos.ciclo', '1002');
                    } elseif ($value === '3') {
                        $builder->where('ciclo.ciclo', '1003');
                    } elseif ($value === '4') {
                        $builder->where('ciclo.ciclo', '1004');
                    } elseif ($value === '5') {
                        $builder->where('ciclo.ciclo', '1005');
                    } elseif ($value === '6') {
                        $builder->where('ciclo.ciclo', '1006');
                    } elseif ($value === '7') {
                        $builder->where('ciclo.ciclo', '1007');
                    } elseif ($value === '8') {
                        $builder->where('ciclo.ciclo', '1008');
                    } elseif ($value === '9') {
                        $builder->where('ciclo.ciclo', '1009');
                    } elseif ($value === '10') {
                        $builder->where('ciclo.ciclo', '1010');
                    } elseif ($value === '11') {
                        $builder->where('ciclo.ciclo', '1011');
                    } elseif ($value === '12') {
                        $builder->where('ciclo.ciclo', '1012');
                    }
                }),
            SelectFilter::make('Anomalias')
                ...
                ->filter(function (Builder $builder, $value) {
                    if ($value === '1') {
                        ...
                        $builder->whereJsonContains('reportesverificacions.anomalia', '68');
                    } elseif ($value === '15') {
                        $builder->whereJsonContains('reportesverificacions.anomalia', '71');
                    } elseif ($value === '16') {
                        $builder->whereJsonContains('reportesverificacions.anomalia', '72');
                    } elseif ($value === '17') {
                        $builder->whereJsonContains('reportesverificacions.anomalia', '73');
                    } elseif ($value === '18') {
                        $builder->whereJsonContains('reportesverificacions.anomalia', '74');
                    }
                }),
        ];
    }
            ...
            Column::make("Estado", "revisado")
                ->format(
                    fn ($value, $row, Column $column) => match ($value) {
                        0 => '<span class="badge badge-success">Encontrado</span>',
                        1 => '<span class="badge badge-danger">No Encontrado</span>',
                        default =>'<span class="badge badge-warning">Sin Auditar</span>'
                    }
                )
                ->html()
                ->collapseOnMobile(),
